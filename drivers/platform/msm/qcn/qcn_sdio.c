...
static struct qcn_sdio_rw_info *qcn_sdio_alloc_rw_req(void)
{
	...
	rw_req = list_first_entry(&sdio_ctxt->rw_free_q, struct qcn_sdio_rw_info,
				  list);
...
static int qcn_sdio_recv_buff(u32 cid, void *buff, int len)
{
	int ret = 0;
...
static void qcn_sdio_rw_work(struct work_struct *work)
{
	...
	while (1) {
		...
		if (atomic_read(&sdio_ctxt->ch_status[rw_req->cid]) == -1) {
		...
		do {
			ret = atomic_cmpxchg(&sdio_ctxt->ch_status[rw_req->cid],
									0, 1);
			if (ret) {
				if (ret == -1) {
					qcn_sdio_free_rw_req(rw_req);
...
int qcn_sdio_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
	...
	sdio_ctxt = (struct qcn_sdio *)kzalloc(sizeof(struct qcn_sdio),
								GFP_KERNEL);
	if (!sdio_ctxt)
		return -ENOMEM;
...
static int qcn_sdio_plat_remove(struct platform_device *pdev)
{
	...
	while (!list_empty(&cinfo_head)) {
		cinfo = list_first_entry(&cinfo_head, struct qcn_sdio_client_info,
								cli_list);
		mutex_unlock(&lock);
...
{
	struct qcn_sdio_ch_info	*ch_info = NULL;
	struct qcn_sdio_client_info	*cli_info = NULL;
	...
	cli_info = container_of(handle, struct qcn_sdio_client_info, cli_handle);

	while (!list_empty(&cli_info->ch_head)) {
		ch_info = list_first_entry(&cli_info->ch_head, struct qcn_sdio_ch_info,
								ch_list);
		sdio_al_deregister_channel(&ch_info->ch_handle);
	}
	mutex_lock(&lock);
		...
		struct sdio_al_channel_data *channel_data)
{
	...
	ch_info = (struct qcn_sdio_ch_info *)kzalloc(sizeof(struct qcn_sdio_client_info),
								GFP_KERNEL);
	if (!ch_info) {
		pr_err("%s: SDIO: No memory\n", __func__);
		return NULL;
	...
	cli_info = container_of(client_handle, struct qcn_sdio_client_info,
								cli_handle);
	ch_info->ch_handle.channel_data = &ch_info->ch_data;
	ch_info->chandle = &cli_info->cli_handle;
	list_add_tail(&ch_info->ch_list, &cli_info->ch_head);
		...
		void *buf, size_t len, int priority, void *ctxt)
{
	...
	if (!(cid < QCN_SDIO_CH_MAX) && (atomic_read(&sdio_ctxt->ch_status[cid])
									< 0)) {
		return -EINVAL;
	}
